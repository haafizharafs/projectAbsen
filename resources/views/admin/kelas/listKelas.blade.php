                    ...
                    <tbody>
                        @foreach ($kelas as $kelas)
                            <tr>
                                ...
                                <td>{{ $kelas->nama }}</td>
                                <td>
                                    <a href="{{ route('admin.editKelas', $kelas->id) }}" class="btn btn-warning">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <!-- Button trigger modal -->
                                    <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                        data-bs-target="#modalDelete-{{ $kelas->id }}">
                                        <i class="fas fa-trash"></i>
                                    </button>

                                    <!-- Modal -->
                                    <div class="modal fade" id="modalDelete-{{ $kelas->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    Yakin Ingin Menghapus Semester {{ $kelas->nama }}?
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                        data-bs-dismiss="modal">Batal</button>
                                                    <form action="{{ route('admin.deleteKelas', $kelas->id) }}"
                                                        method="post">
                                                        @csrf
